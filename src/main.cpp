...
static void drawMenu() {
    ...
    if (ImGui::CollapsingHeader("ESP", ImGuiTreeNodeFlags_DefaultOpen)) {
        ImGui::Checkbox("Enable ESP", &g_settings.esp);
    ...
    if (ImGui::CollapsingHeader("Debug Info", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        ImGui::Text("BaseNetworkable: 0x%llX", (unsigned long long)base_networkable::base_address);
        ImGui::Text("Camera: 0x%llX", (unsigned long long)main_camera::base_address);
        ...
        if (g_il2cppBase && base_networkable::base_address) {
            ...
            ImGui::Text("Entity List: cnt=%d buf=0x%llX", count, (unsigned long long)buffer);
        }
    }
